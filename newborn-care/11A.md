...
1.  A high flow 220 volt electric air pump (uses a fish tank air pump!) with a power cable
2.  A pressure regulator consisting of a water bottle
3.  An oxygen inlet port where oxygen can be provided from piped wall oxygen, an oxygen cylinder or an oxygen concentrator.
4.  An oxygen flow meter which controls the flow of oxygen if needed
5.  A total flow meter which controls the flow of mixed (blended) gas from the device to the infant
6.  Tubing from the outflow port to the infant
7.  Nasal prongs (different sizes) with two elbow connectors and an end cap
...
Illustration of the nasal prongs in place needed here
...
If the total flow rate has to be increased the oxygen flow rate will also need to be increased to keep the FiO2 the same. It is best to use a saturation monitor to decide what oxygen flow rate is needed.  A higher oxygen flow rate will give a higher FiO2.
...
Increase the oxygen flow rate if the FiO2 remains low
